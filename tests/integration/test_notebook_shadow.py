...
from data_layer.feature_store.feature_engineer import FeatureEngineer as CanonicalFeatureEngineer

# Repo-relative path to the archived research notebook. Walk up from this file
# until we find the ancestor that contains research/ (depth-independent, so the
# test can live at any level under tests/).
def _find_repo_root() -> str:
    here = os.path.dirname(os.path.abspath(__file__))
    while os.path.dirname(here) != here:
        if os.path.isdir(os.path.join(here, "research")):
            return here
        here = os.path.dirname(here)
    return os.path.dirname(os.path.abspath(__file__))


NOTEBOOK_PATH = os.path.join(_find_repo_root(), "research", "notebooks", "AutoData_Analyst_v1_aymen.ipynb")
