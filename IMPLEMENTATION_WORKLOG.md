...
## 2026-04-04: Brain Module Contract Extraction

- Created `src/brain/` package for notebook-extracted logic intended for both Python runtime and future native bindings.
- Added `src/brain/features.py` with pure feature/target functions:
  - `calculate_std(...)`
  - `calculate_atr(...)`
  - `add_3sigma_target(...)`
- Added `src/brain/models.py` with pure XGBoost dispatcher flow:
  - `train_xgboost_classifier(...)`
  - `predict_xgboost_classifier(...)`
  - `xgboost_dispatcher(...)`
- Added `src/brain/__init__.py` as package contract surface:
  - `BrainTargetConfig` and `BrainTrainingConfig` dataclasses
  - `build_brain_frame(...)` for consistent ATR/std/target assembly
  - `train_breakout_dispatcher(...)` for model training/prediction entrypoint
  - Explicit `__all__` exports to define integration boundary.
